...
class LedMultiplex:
    def __init__(self, rows, cols, led_duty_cycle):
        self.cols = cols
        self.rows = rows
    ...
    def flicker_led(self, row, col, length):
        self._reset_state()
        period = length / 10
        for i in range(10):
            self._led_on(self.rows[row], self.cols[col])
            time.sleep(period / 2)
            self._led_off(self.rows[row], self.cols[col])
            time.sleep(period / 2)

    def run_leds(self, cycle_seconds):
        self._stopped = False
        self._reset_state()
        self._run = True

        led_count = len(self.rows) * len(self.cols)
        led_time = cycle_seconds / led_count
